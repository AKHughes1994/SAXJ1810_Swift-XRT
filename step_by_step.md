...
4. To convert from a count rate to a flux first make a spectrum, arf, and rmf file. Load in the spectrum and the arf (using `arf [ARFFILE]`) and rmf (using `rmf [RMFFILE]`) into xspec. Then pick a simple tbabs*pegpwrlw model (fill in the known NH), and from there you can predict a count rate using `show rates`, record the model predicted rate for the assumed model flux (by default just use 1e-12) and from there you can convert the count rate to flux, and you have your upper limit.  
